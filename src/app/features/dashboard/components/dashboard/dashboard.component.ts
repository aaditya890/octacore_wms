...
export class DashboardComponent {
  ...
  async ngOnInit() {
    ...
    console.log('inventory total items', this.stats.totalItems);
  }
  ...
  async loadDashboardData() {
    ...
    try {
      ...
      const transactions = await this.inoutService.getTransactionListPaged(0, 100, {});
      ...
      const lowStock = allItems.filter((i: any) => {
        ...
        const minQty = parseFloat(i.min_quantity ?? 0);
        return qty <= (minQty || 10); // 👈 default 10 threshold if min=0
      });
      ...
      for (const t of transactions.data || []) {
        ...
        if (t.transaction_type === 'inward') inward += value;
        if (t.transaction_type === 'outward') outward += value;
        if (date === today) todayValue += value;
      }

      this.stats.totalInwards = inward;
      this.stats.totalOutwards = outward;
